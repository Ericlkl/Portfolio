...
import {
  DevelopmentTool,
  DevelopmentToolIncludes,
} from 'interfaces/models/devTools';
...
const compareDevToolFunction: CompareFunction<DevelopmentTool> = (
  pivot,
  currentElm
) => {
...
export class DevelopmentToolManager {
  static async getDevelopmentTools({
    ...
    if (sortedByField) {
      return quickSort(devTools, compareDevToolFunction);
    }
    return devTools;
  }
}
